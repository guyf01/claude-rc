...
- **Decoy** — exists to make the target tier look like the obvious deal. Priced close to the target but with noticeably less value. The principle: humans don't price in isolation — they decide by comparison. A two-option page trains customers to pick the cheap one (tested: 70% do). Add a decoy between them and nobody buys it — but the premium option jumps to 84%, with zero product changes. The decoy sold nothing and did everything. Price it near the premium tier, give it clearly less, and the premium stops feeling expensive — it starts feeling smart. Risk: sophisticated B2B buyers may spot intentional decoys and it can erode trust.
