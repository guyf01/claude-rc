...
- **Missing indexes** — columns used in WHERE, ORDER BY, JOIN conditions. Grep for raw SQL or ORM `where()` calls, then search migration files or schema definitions to verify if the column is indexed. If no schema is found in the repo, flag it as "could not verify — check manually".
